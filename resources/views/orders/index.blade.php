	...
	<div class="container py-12">
		<section class="grid grid-cols-5 gap-6 text-white">
			<a href="{{ route('orders.index') . '?status=5' }}" class="bg-red-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
				<p class="text-center text-2xl">{{ $countStatus['anulado'] }}</p>
			...
			<a href="{{ route('orders.index') . '?status=1' }}" class="bg-gray-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
				<p class="text-center text-2xl">{{ $countStatus['pendiente'] }}</p>
			...
			<a href="{{ route('orders.index') . '?status=2' }}" class="bg-blue-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
				<p class="text-center text-2xl">{{ $countStatus['pagado'] }}</p>
			...
			<a href="{{ route('orders.index') . '?status=3' }}" class="bg-yellow-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
				<p class="text-center text-2xl">{{ $countStatus['enviado'] }}</p>
			...
			<a href="{{ route('orders.index') . '?status=4' }}" class="bg-green-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
				<p class="text-center text-2xl">{{ $countStatus['entregado'] }}</p>
						...
						<a href="{{ route('orders.show', $order) }}" class="flex items-center py-2 px-4 hover:bg-gray-100">
							<span class="w-12 text-center">
								@switch($order->status)
									...
									@case(4)
										<i class="fas fa-check-cricle text-green-500 opacity-50"></i>
									@break
